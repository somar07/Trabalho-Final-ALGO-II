...
int main(int argc, char const *argv[]){
    ...
    // CADA INDICIE CORRESPONDE A UMA INSTANCIA DO ARQUIVO.
        separa_palavras_instancia(0);
        printf("instancia = %s", dados[0].SENTENCA);
...
void separa_palavras_instancia( int instancia){
    ...
    for(i = 0; i < j ; i++){
        //printf("\nTempo espaço => %d|| Tempo seguinte => %d\n",dados[instancia].VETOR_TEMPO[espaco[i]],dados[instancia].VETOR_TEMPO[espaco[i]+1]);
        if(((dados[instancia].VETOR_TEMPO[espaco[i]] - dados[instancia].VETOR_TEMPO[espaco[i]+1])* (-1))<= 20){
            ...
            */
            printf("\nBusca 1\n");
            printf("\nPalavras antes da troca P1 ==== %s|| P2 ==== %s\n",P1,P2);
            result_busca = procura(P1, P2, instancia);
            printf("\nResultado ==== %d\n",result_busca);
            //printf("\n P1 Original = %s, P2 Original = %s", P1, P2);
            //printf("\n Dicionario: = %s", dados[instancia].DICIONARIO);
...
void correcao_troca( char string1[],char string2[], int instancia, int space){
   ...
   // printf("\n P1 Modificada = %s, P2 Modificada = %s", string1, string2);
   // printf("\n------------------------------\n");
    ...
    if(altera == 1){
      ...
      //  printf("STRING {%s}\n", STRING);
        ...
        for(int i = 0; i < strlen(STRING); i++){
             printf("\nAntes MSENTENCA====Posi[%d] =%c\n",posicao,dados[instancia].SENTENCA[i]);
             printf("\nAntes STRING grande === %c\n",STRING[i]);
            dados[instancia].SENTENCA[posicao] = STRING[i];
            posicao++;
            printf("\nDepois MSENTENCA==== Posi [%d] =%c\n",posicao,dados[instancia].SENTENCA[i]);
             printf("\nDepois STRING grande === %s\n",STRING);

        }

        for(int i = 0; i < strlen(STRING); i++){
            if(STRING[i] == ' '){
             //   printf("\n Espaco da STRING: Pos = %d\n", i);
                posicao = i +1;
                printf("\n====================================================\n");
                printf("\nDepois MSENTENCA==== %s\n",dados[instancia].SENTENCA);
             printf("\nDepois STRING grande === %c\n",STRING[i]);
            }
        }
...
int procura(char string1[],char string2[], int instancia){
    ...
    for(int i = 0; i < tamDicionario; i++){
        ...
        //printf("\n Procura P1 ?- %s, %s.\n", string1, separacao_palavras_dicionario[i]);
        //printf("\n count_1 %d",count_1);
        if(count_1 == 1){
            printf("\n P1 OK");
            break;
   ...
   // printf("\n----------------------------------------------------");
    ...
    for(int i = 0; i < tamDicionario; i++){
       ...
       // printf("\n Procura P2 ?- %s, %s.\n", string2, separacao_palavras_dicionario[i]);
        // printf("\n count_2 %d",count_2);
        if(count_2 == 1){
            printf("\n P2 OK");
            break;
...
void separaMatrizDicionario(int instancia, int tamDicionario){
    ...
    for(int m = 0; m < strlen(dados[instancia].DICIONARIO); m++){
...
/* BACKUP SEPARA PALAVRAS DICIONARIO PARA MATRIZ 
void separaMatrizDicionario(int instancia, int tamDicionario){
    int i,j,k;
    //SEPARA AS PALAVRAS DO DICIONARIO NUMA MATRIZ.
